app.initNav = async function (currentConfigs, routerInstance) {
	let template = await fetch(APP_BASE + "/js/nav/view.html").then(owo => owo.text())

	let version = await fetch(APP_BASE + "/build.txt").then(owo => owo.text())
	...
	let view = proxymity(template, {
		...
		goToDownloads: () => routerInstance.rout("/downloads"),
